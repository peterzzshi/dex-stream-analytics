package events

import "strings"

type EventType string

type Event interface {
	GetEventType() EventType
	GetEventID() string
	GetPairAddress() string
	GetEventTimestamp() int64
	GetBlockNumber() int64
	ToMap() map[string]interface{}
}

type BaseEvent struct {
	EventType       EventType `json:"eventType"`
	EventID         string    `json:"eventId"`
	BlockNumber     int64     `json:"blockNumber"`
	BlockTimestamp  int64     `json:"blockTimestamp"`
	TransactionHash string    `json:"transactionHash"`
	LogIndex        int32     `json:"logIndex"`
	PairAddress     string    `json:"pairAddress"`
	Token0          string    `json:"token0"`
	Token1          string    `json:"token1"`
	Token0Symbol    *string   `json:"token0Symbol,omitempty"`
	Token1Symbol    *string   `json:"token1Symbol,omitempty"`
	EventTimestamp  int64     `json:"eventTimestamp"`
}

const (
	EventTypeSwap     EventType = "Swap"
	EventTypeMint     EventType = "Mint"
	EventTypeBurn     EventType = "Burn"
	EventTypeTransfer EventType = "Transfer"
)

var (
	_ Event = (*SwapEvent)(nil)
	_ Event = (*MintEvent)(nil)
	_ Event = (*BurnEvent)(nil)
	_ Event = (*TransferEvent)(nil)
)

func (et EventType) CloudEventType() string {
	return "com.dex.events." + strings.ToLower(string(et))
}

func (base BaseEvent) GetEventType() EventType  { return base.EventType }
func (base BaseEvent) GetEventID() string       { return base.EventID }
func (base BaseEvent) GetPairAddress() string   { return base.PairAddress }
func (base BaseEvent) GetEventTimestamp() int64 { return base.EventTimestamp }
func (base BaseEvent) GetBlockNumber() int64    { return base.BlockNumber }

// ToMap produces the field map consumed by Avro serialization.
func (base BaseEvent) ToMap() map[string]interface{} {
	return map[string]interface{}{
		"eventId":         base.EventID,
		"blockNumber":     base.BlockNumber,
		"blockTimestamp":  base.BlockTimestamp,
		"transactionHash": base.TransactionHash,
		"logIndex":        base.LogIndex,
		"pairAddress":     base.PairAddress,
		"token0":          base.Token0,
		"token1":          base.Token1,
		"token0Symbol":    toNullable(base.Token0Symbol),
		"token1Symbol":    toNullable(base.Token1Symbol),
		"eventTimestamp":  base.EventTimestamp,
	}
}

// AllEventTypes returns a defensive copy to prevent mutation of the canonical set.
func AllEventTypes() []EventType {
	return []EventType{
		EventTypeSwap,
		EventTypeMint,
		EventTypeBurn,
		EventTypeTransfer,
	}
}

// toNullable wraps pointer values for Avro union encoding: nil -> null, *T -> {"type": value}.
func toNullable[T any](ptr *T) interface{} {
	if ptr == nil {
		return nil
	}

	value := any(*ptr)
	switch typed := value.(type) {
	case string:
		return map[string]interface{}{"string": typed}
	case float64:
		return map[string]interface{}{"double": typed}
	default:
		return value
	}
}
